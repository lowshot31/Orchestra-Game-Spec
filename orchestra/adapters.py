    ...
    def generate(self, task: str, idea: str, context: str = "", intervention: str = "") -> str:
        ...
        if task == "rebuttal":
            return (
                "# 디자이너 반론\n\n"
                f"{steering_notes}"
                "- 리뷰어의 병합 깊이 제한 의견에 동의합니다. 최대 3단계로 제한하겠습니다.\n"
                "- 다만 보상 구조를 완전히 빼는 것은 반대합니다. 세션 완료 보상은 리텐션의 핵심입니다.\n"
                "- 절충안: 단순 코인 보상 1종만 MVP에 포함하고, 복합 보상은 후순위로 미루겠습니다.\n"
                "- 엣지 케이스 처리는 리뷰어 제안대로 병합 불가 상태 감지 로직을 추가하겠습니다.\n"
            )
        ...
        if task == "ceo_review":
            return (
                ...
                "- 디자이너와 리뷰어의 절충안(보상 1종 포함)에 동의합니다.\n"
